...
impl CertificateBuilder {
    pub fn new(qcli: QCli) -> Self {
        Self { qcli }
    }

    pub fn new_signed_stake_pool_cert(self) -> SignedStakePoolCertBuilder {
        SignedStakePoolCertBuilder::new(self.qcli)
    }
    ...
    pub fn new_signed_vote_plan<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        let cert = self.qcli.certificate().new_vote_plan(proposal_file);
...
impl SignedStakePoolCertBuilder {
    ...
    pub fn build(self) {
        ...
        let stake_pool_cert = self.qcli.certificate().new_stake_pool_registration(
            &self.pool_kes_pk,
            &self.pool_vrf_pk,
            self.start_validity,
