use crate::quibitous::grpc::read_into;
...
pub struct QuibitousClient {
    addr: SocketAddr,
    inner_client: NodeClient<Channel>,
    rt: Runtime,
}

impl Clone for QuibitousClient {
    fn clone(&self) -> Self {
        QuibitousClient::new(self.addr)
    }
}

impl fmt::Debug for QuibitousClient {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("QuibitousClient")
            .field("host", &self.addr)
            .finish()
    }
}

impl QuibitousClient {
...
pub struct QuibitousWatchClient {
    addr: SocketAddr,
    inner_client: WatchClient<Channel>,
    rt: Arc<Runtime>,
}

impl Clone for QuibitousWatchClient {
    fn clone(&self) -> Self {
        QuibitousWatchClient::new(self.addr)
    }
}

impl fmt::Debug for QuibitousWatchClient {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("QuibitousWatchClient")
            .field("host", &self.addr)
            .finish()
    }
}
