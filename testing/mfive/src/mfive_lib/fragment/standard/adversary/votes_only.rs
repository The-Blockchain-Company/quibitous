...
use crate::mfive_lib::DiscriminationExtensions;
use crate::mfive_lib::{args::parse_shift, build_monitor, MfiveError};
use chain_addr::Discrimination;
use chain_impl_mockchain::block::BlockDate;
use quibitous_automation::quibitous::RemoteQuibitousBuilder;
use quibitous_automation::testing::block0::{get_block, Block0ConfigurationExtension};
...
impl VotesOnly {
    pub fn exec(&self) -> Result<(), MfiveError> {
        ...
        let remote_quibitous = RemoteQuibitousBuilder::new("node".to_owned())
            .with_rest(self.endpoint.parse().unwrap())
            .build();

        let settings = remote_quibitous.rest().settings().unwrap();
        ...
        let generator = AdversaryVoteCastsGenerator::new(
            ...
            faucet,
            vote_plans,
            remote_quibitous.clone_with_rest(),
            transaction_sender,
        );
