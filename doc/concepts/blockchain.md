...
* Shardagnostic BFT (OBFT)
* Shardagnostic Genesis-Optimum

Shardagnostic BFT is a simple Byzantine Fault Tolerant (BFT) protocol where the
block makers is a known list of leaders that successively create a block and
broadcast it on the network.

Shardagnostic Genesis Optimum is a proof of stake (PoS) protocol where the block
...
In Genesis-Optimum slot time duration is constant, however the frequency of
creating blocks is not stable, since the creation of blocks is a probability
that is linked to the stake and consensus_genesis_optimum_active_slot_coeff.

**Note**: In Genesis-Optimum, if there is no stake in the system, no blocks will be
...
key, whereas in Genesis-Optimum mode, the leader is a stake pool.
