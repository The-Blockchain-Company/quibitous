...
pub fn qcli_e2e_flow_private_vote() {
    ...
    let communication_sk = qcli.votes().committee().communication_key().generate();
    let communication_pk = qcli
        .votes()
        .committee()
        .communication_key()
    ...
    let election_public_key = qcli.votes().election_public_key(member_pk.clone());
    ...
    let yes_vote_cast = qcli.certificate().new_private_vote_cast(
        vote_plan_id.clone(),
        0,
        yes_choice,
        3,
    ...
    let no_vote_cast = qcli.certificate().new_private_vote_cast(
        vote_plan_id.clone(),
        0,
        no_choice,
        3,
    ...
    let active_plans = quibitous.rest().vote_plan_statuses().unwrap();
    let active_plans_file = temp_dir.child("active_plans.json");
    active_plans_file
        .write_str(&serde_json::to_string(&active_plans).unwrap())
        .unwrap();

    let decryption_shares = qcli.votes().tally().decryption_shares(
    ...
    let result = qcli.votes().tally().decrypt_results(
        active_plans_file.path(),
        &vote_plan_id,
    ...
    let vote_tally_cert = qcli.certificate().new_private_vote_tally(
        result_file.path(),
        vote_plan_id,
        merged_shares_file.path(),
    );
...
#[test]
pub fn qcli_private_vote_invalid_proof() {
    let qcli: JCli = Default::default();
    let temp_dir = TempDir::new().unwrap().into_persistent();
    let wallet_initial_funds = 1_000_000;
    ...
    qcli.votes().tally().decrypt_results_expect_fail(
        active_plans_file.path(),
        &vote_plan_id,
        merged_shares_file.path(),
        1,
