...
use quibitous_automation::testing::observer::{Event, Observable, Observer};
...
impl MonitorController {
    ...
    pub fn spawn_node(
        ...
        self.spawn_node_custom(
            self.new_spawn_params(node_alias)
                ...
                .quibitous(self.session_settings.quibitous.to_path_buf()),
        )
    }

    pub fn spawn_node_custom(&mut self, input_params: SpawnParams) -> Result<Node, Error> {
        let quibitous_process = self.inner.spawn(input_params.clone())?;

        let progress_bar =
            self.build_progress_bar(input_params.get_alias(), quibitous_process.rest_address());

        Ok(Node::new(quibitous_process, progress_bar))
    }
    ...
    pub fn spawn_legacy_node(
        ...
        let (quibitous_process, legacy_node_config) =
