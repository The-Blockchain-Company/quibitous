...
use quibitous_automation::{qcli::JCli, testing::keys::create_new_key_pair};
use quibitous_lib::interfaces::Certificate;
...
#[test]
pub fn qcli_creates_correct_retirement_certificate() {
    ...
    let certificate = qcli.certificate().new_stake_pool_registration(
    ...
    let stake_pool_id = qcli.certificate().stake_pool_id(input_file.path());

    let expected_certificate = qcli.certificate().new_stake_pool_retirement(&stake_pool_id);
    ...
    let stake_pool_id_from_retirement = qcli
        .certificate()
