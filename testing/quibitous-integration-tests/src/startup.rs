...
use quibitous_automation::quibitous::{
    ConfigurationBuilder, QuibitousProcess, SecretModelFactory, Starter, StartupError,
};
use quibitous_lib::{
...
) -> Result<(QuibitousProcess, Vec<StakePool>), StartupError> {
...
pub fn start_bft(
    ...
    config_builder: &mut ConfigurationBuilder,
) -> Result<QuibitousProcess, StartupError> {
    let temp_dir = TempDir::new()?;
