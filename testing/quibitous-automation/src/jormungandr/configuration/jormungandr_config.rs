...
use quibitous_lib::interfaces::{Address, Block0Configuration, NodeConfig, UTxOInfo};
use serde::Serialize;
use std::fs::File;
...
#[derive(Debug, Clone)]
pub struct QuibitousParams<Conf = NodeConfig> {
    node_config: Conf,
...
impl<Conf: TestConfig> QuibitousParams<Conf> {
...
impl<Conf: TestConfig + Serialize> QuibitousParams<Conf> {
