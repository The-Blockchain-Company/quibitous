...
use crate::quibitous::{
    grpc::QuibitousClient, rest::uri_from_socket_addr, FragmentNode, FragmentNodeError,
    QuibitousLogger, QuibitousRest, LogLevel, MemPoolCheck,
};
...
pub struct RemoteQuibitous {
    rest: Option<QuibitousRest>,
    grpc: Option<QuibitousClient>,
    logger: Option<QuibitousLogger>,
    node_config: Option<NodeConfig>,
    alias: NodeAlias,
}

impl RemoteQuibitous {
    pub fn new(
        rest: Option<QuibitousRest>,
        grpc: Option<QuibitousClient>,
        logger: Option<QuibitousLogger>,
...
impl SyncNode for RemoteQuibitous {
...
impl FragmentNode for RemoteQuibitous {
    fn alias(&self) -> NodeAlias {
        self.alias().to_string()
    }
...
pub struct RemoteQuibitousBuilder {
    rest: Option<QuibitousRest>,
    grpc: Option<QuibitousClient>,
    logger: Option<QuibitousLogger>,
    node_config: Option<NodeConfig>,
...
impl RemoteQuibitousBuilder {
    ...
    pub fn with_rest(self, address: SocketAddr) -> Self {
        self.with_rest_client(QuibitousRest::new(uri_from_socket_addr(address)))
    }

    pub fn with_rest_client(mut self, client: QuibitousRest) -> Self {
        self.rest = Some(client);
        self
    }

    pub fn with_grpc<S: Into<String>>(mut self, address: S) -> Self {
        self.grpc = Some(QuibitousClient::from_address(&address.into()).unwrap());
        self
    }

    pub fn with_logger(&mut self, mut process: Child) -> &mut Self {
        self.logger = Some(QuibitousLogger::new(
    ...
    pub fn with_node_config(mut self, node_config: PathBuf) -> Self {
        self.node_config =
            Some(serde_yaml::from_str(&quibitestkit::file::read_file(node_config)).unwrap());
        self
    }

    pub fn build(self) -> RemoteQuibitous {
        RemoteQuibitous::new(
            self.rest,
            self.grpc,
            self.logger,
