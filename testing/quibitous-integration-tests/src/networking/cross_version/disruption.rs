...
pub fn last_nth_release(#[case] n: u32) {
    ...
    let legacy_app = get_quibitous_bin(last_release, &session_settings.root.child("quibitous"));
...
fn test_legacy_release(legacy_app: PathBuf, version: Version) {
    ...
    let (leader1, _) = controller
        .spawn_legacy(SpawnParams::new(LEADER_1).quibitous(legacy_app), &version)
        .unwrap();
...
fn test_legacy_disruption_release(legacy_app: PathBuf, version: Version) {
    ...
    let (leader1, _) = controller
        .spawn_legacy(
            SpawnParams::new(LEADER_1).quibitous(legacy_app.clone()),
            &version,
        )
        .unwrap();
...
pub fn newest_node_enters_legacy_network() {
    ...
    let (leader2, _) = controller
        .spawn_legacy(
            SpawnParams::new(LEADER_2).quibitous(legacy_app.clone()),
            &last_release.version(),
        )
        .unwrap();
    ...
    let (leader3, _) = controller
        .spawn_legacy(
            SpawnParams::new(LEADER_3).quibitous(legacy_app.clone()),
            &last_release.version(),
        )
        .unwrap();
    ...
    let (old_leader4, _) = controller
        .spawn_legacy(
            SpawnParams::new(LEADER_4).quibitous(legacy_app),
            &last_release.version(),
        )
        .unwrap();
