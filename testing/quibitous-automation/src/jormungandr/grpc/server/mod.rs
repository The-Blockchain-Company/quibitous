...
impl From<ConsensusVersion> for ProtocolVersion {
    fn from(from: ConsensusVersion) -> Self {
        match from {
            ...
            ConsensusVersion::GenesisOptimum => Self::GenesisOptimum,
        }
    }
}
...
pub struct QuibitousServerImpl {
    data: Arc<RwLock<MockServerData>>,
}

impl QuibitousServerImpl {
    pub fn new(data: Arc<RwLock<MockServerData>>) -> Self {
        ...
        QuibitousServerImpl { data }
    }
}

#[tonic::async_trait]
impl Node for QuibitousServerImpl {
    ...
    async fn peers(
        ...
        let data = self.data.read().unwrap();
        // Gossip struct serde, quibitous/src/topology/gossip.rs
