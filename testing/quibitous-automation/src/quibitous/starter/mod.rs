...
use crate::quibitous::{ConfigurationBuilder, QuibitousError, QuibitousProcess, RestError};
use crate::{
    quibitous::{
        QuibitousParams, LegacyConfigConverter, LegacyConfigConverterError, LegacyNodeConfig,
        TestConfig, Version,
    },
    testing::{configuration::get_quibitous_app, SpeedBenchmarkDef, SpeedBenchmarkRun},
};
...
pub enum StartupError {
    #[error("could not start quibitous due to process issue")]
    QuibitousNotLaunched(#[from] ProcessError),
    #[error("error setting up temporary filesystem fixture")]
    FsFixture(#[from] FixtureError),
    #[error("failed to convert quibitous configuration to a legacy version")]
    ...
    #[error("too many failures while attempting to start quibitous")]
    TooManyAttempts,
...
pub struct Starter {
    timeout: Duration,
    quibitous_app_path: Option<PathBuf>,
    sleep: u64,
...
impl Starter {
    ...
    pub fn quibitous_app(&mut self, path: PathBuf) -> &mut Self {
        self.quibitous_app_option(&Some(path))
    }

    pub fn quibitous_app_option(&mut self, path: &Option<PathBuf>) -> &mut Self {
        self.quibitous_app_path = path.clone();
        self
    }
    ...
    pub fn config(&mut self, config: QuibitousParams) -> &mut Self {
    ...
    pub fn build_configuration(
        &mut self,
    ) -> Result<(QuibitousParams, Option<TestingDirectory>), StartupError> {
    ...
    pub fn start_async(&mut self) -> Result<QuibitousProcess, StartupError> {
        let (params, temp_dir) = self.build_configuration()?;
        if let Some(version) = self.legacy.as_ref() {
            Ok(
                ConfiguredStarter::legacy(self, version.clone(), params, temp_dir)?
    ...
    pub fn start(&mut self) -> Result<QuibitousProcess, StartupError> {
        let (params, temp_dir) = self.build_configuration()?;
...
impl<'a> ConfiguredStarter<'a, NodeConfig> {
    pub fn new(
        starter: &'a Starter,
        params: QuibitousParams<NodeConfig>,
        temp_dir: Option<TestingDirectory>,
    ) -> Self {
        ConfiguredStarter {
            starter,
    ...
    Conf: TestConfig + Serialize + Debug,
{
    ...
    fn start_with_fail_in_stderr(self, expected_msg: &str) {
        get_command(
            ...
            &self
                ...
                .quibitous_app_path
                .clone()
                .unwrap_or_else(get_quibitous_app),
    ...
    fn start_process(&self) -> Child {
        ...
        command
            .spawn()
            .expect("failed to execute 'start quibitous node'")
    }
    ...
    fn start_async(self) -> Result<QuibitousProcess, StartupError> {
        QuibitousProcess::new(
            self.start_process(),
    ...
    fn start(mut self) -> Result<QuibitousProcess, StartupError> {
        let mut retry_counter = 1;
        loop {
            let process = self.start_process();

            let mut quibitous = QuibitousProcess::new(
                process,
                self.params.node_config(),
                self.params.block0_configuration().clone(),
                self.temp_dir.take(),
            ...
            match (
                ...
                (Ok(()), _) => {
                    cond_println!(self.starter.verbose, "quibitous is up");
                    return Ok(quibitous);
                }

                (
                    Err(StartupError::QuibitousError(QuibitousError::PortAlreadyInUse)),
                ...
                (Err(err), OnFail::Panic) => {
                    panic!("Quibitous node cannot start due to error: {}", err);
                }
                (Err(err), _) => {
                    cond_println!(
                        self.starter.verbose,
                        "Quibitous failed to start due to error {:?}. Retrying... ",
                        err
                    );
                    retry_counter -= 1;
                }
            ...
            self.temp_dir = quibitous.steal_temp_dir();
        }
    }
}
