...
use quibitous_lib::interfaces::Subscription;
use quibitous_lib::time::SystemTime;
use serde::Serialize;
...
impl From<quibitous_lib::interfaces::NodeId> for NodeId {
    fn from(id: quibitous_lib::interfaces::NodeId) -> Self {
        let id_bytes = id.as_ref().as_ref();
        NodeId(id_bytes.try_into().unwrap())
    }
}

impl From<NodeId> for quibitous_lib::interfaces::NodeId {
    fn from(node_id: NodeId) -> quibitous_lib::interfaces::NodeId {
        quibitous_lib::interfaces::NodeId::from_hex(&node_id.0.to_string()).unwrap()
    }
}
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        Ok(Self::from(
            PublicKey::<Ed25519>::from_binary(bytes)
                .map(quibitous_lib::interfaces::NodeId::from)?,
        ))
    }
}
