...
pub enum LeadershipError {
    ...
    TooEarlyForTimeFrame {
        time: quibitous_lib::time::SystemTime,
    ...
    async fn action_run_entry_build_block(
        ...
        let signing = {
            let ver = match event.output {
                ...
                LeaderOutput::GenesisOptimum(..) => BlockVersion::KesVrfproof,
            };
            ...
            match output {
                ...
                LeaderOutput::GenesisOptimum(node_id, vrfproof) => {
                    let block = block_builder(ver, contents, |hdr_builder| {
                        let final_builder = hdr_builder
                            .set_parent(&parent_id, chain_length)
                            .set_date(date)
                        ...
                        enclave
                            .query_header_genesis_optimum_finalize(final_builder)
                            .map(|h| h.generalize())
                    });
