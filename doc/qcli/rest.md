...
Quibitous comes with a CLI client for manual communication with nodes over HTTP.
...
`https://` prefix. E.g. `-h http://127.0.0.1`, `--host https://node.com:8443/bcc/api`
...
```sh
qcli rest v0 node stats get <options>
```
...
uptime: 20032
# Node app version
version: quibitous 0.8.9-30d20d2e
```
...
```sh
qcli rest v0 utxo <fragment-id> <output-index> get <options>
```
...
```sh
qcli rest v0 message post <options>
```
...
```sh
qcli rest v0 message logs <options>
```
...
```sh
qcli rest v0 block <block_id> get <options>
```
...
```sh
qcli rest v0 block <block_id> next-id get <options>
```
...
```sh
qcli rest v0 account get <account-id> <options>
```
...
```sh
qcli rest v0 settings get <options>
```
...
```sh
qcli rest v0 shutdown get <options>
```
...
```sh
qcli rest v0 leaders get <options>
```
...
```sh
qcli rest v0 leaders post <options>
```
...
- --debug - see [conventions](#conventions)
- --output-format \<format\> - see [conventions](#conventions)
-f, --file \<file\> - File containing YAML with leader secret. It must have the same format as secret YAML passed to Quibitous as --secret. If not provided, YAML will be read from stdin.
...
```sh
qcli rest v0 leaders delete <id> <options>
```
...
```sh
qcli rest v0 leaders logs get <options>
```
...
```sh
qcli rest v0 stake-pools get <options>
```
...
```sh
qcli rest v0 stake get <options> [<epoch>]
```
...
- `qcli rest v0 stake get <options>` - stake distribution from the current epoch
...
- `qcli rest v0 stake get <options> 10` - stake distribution from a specific epoch (epoch 10 in this example)
...
```sh
qcli rest v0 network stats get <options>
```
...
```sh
qcli rest v0 stake-pool get <pool-id> <options>
```
...
```sh
qcli rest v0 rewards epoch get <epoch> <options>
```
...
```sh
qcli rest v0 rewards epoch get 82 -h <node_addr>
```
...
```sh
qcli rest v0 rewards history get <length> <options>
```
...
```sh
qcli rest v0 rewards history get 2 -h <node_addr>
```
...
```sh
qcli rest v0 vote active committees get <options>
```
...
```sh
qcli rest v0 vote active plans get <options>
```
