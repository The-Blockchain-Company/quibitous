...
pub enum ConfigParam {
    ...
    ConsensusGenesisOptimumActiveSlotsCoeff(ActiveSlotCoefficient),
...
impl From<ConfigParam> for ConfigParamLib {
    fn from(config: ConfigParam) -> Self {
        match config {
            ...
            ConfigParam::ConsensusGenesisOptimumActiveSlotsCoeff(val) => val.into(),
...
impl TryFrom<ConfigParamLib> for ConfigParam {
    ...
    fn try_from(config: ConfigParamLib) -> Result<Self, Self::Error> {
        Ok(match config {
            ...
            config @ ConfigParamLib::ConsensusGenesisOptimumActiveSlotsCoeff(_) => {
                Self::ConsensusGenesisOptimumActiveSlotsCoeff(config.try_into()?)
            }
    ...
    impl Arbitrary for ConfigParam {
        fn arbitrary<G: Gen>(g: &mut G) -> Self {
            match u8::arbitrary(g) % 30 {
                ...
                5 => Self::ConsensusGenesisOptimumActiveSlotsCoeff(Arbitrary::arbitrary(g)),
