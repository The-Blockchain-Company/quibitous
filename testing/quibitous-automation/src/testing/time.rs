...
pub fn wait_for_epoch(epoch: u32, rest: QuibitousRest) {
    wait_for_date(ChainBlockDate { epoch, slot_id: 0 }.into(), rest)
}

pub fn wait_for_date(target_block_date: BlockDate, mut rest: QuibitousRest) {
...
pub fn get_current_date(rest: &mut QuibitousRest) -> BlockDate {
    BlockDate::from_str(
        rest.stats()
            .unwrap()
            .stats
