...
use quibitous_automation::testing::{benchmark_speed, SpeedBenchmarkFinish, SpeedBenchmarkRun};
use quibitous_lib::interfaces::NodeState;

use crate::mfive_lib::bootstrap::ClientLoadConfig;
use crate::mfive_lib::MfiveError;
use quibitous_automation::quibitous::{QuibitousProcess, LogLevel};
use std::{
    thread,
...
impl DurationBasedClientLoad {
    ...
    pub fn run(&self) -> Result<(), MfiveError> {
        ...
        for handle in handles {
            results.push(
                handle
                    .join()
                    .map_err(|_| MfiveError::InternalClientError)?,
            );
        }
    ...
    fn wait_for_bootstrap_phase_completed(
        node: &QuibitousProcess,
        instant: &Instant,
        target: u64,
        benchmark: SpeedBenchmarkRun,
        progress_bar: &ScenarioProgressBar,
    ) -> Result<Option<SpeedBenchmarkFinish>, MfiveError> {
        loop {
    ...
    ) -> Result<thread::JoinHandle<Result<Vec<SpeedBenchmarkFinish>, MfiveError>>, MfiveError>
    {
