...
use quibitous_automation::testing::asserts::VotePlanStatusAssert;
use quibitous_automation::testing::time::{self, wait_for_epoch};
use quibitous_automation::testing::VotePlanExtension;
...
pub fn test_get_committee_id() {
    ...
    let actual_committee_ids = qcli
        .rest()
        .v0()
        .vote()
        .active_voting_committees(&quibitous.rest_uri());
...
pub fn test_get_initial_vote_plan() {
    ...
    let vote_plans = quibitous.rest().vote_plan_statuses().unwrap();
...
pub fn test_vote_flow_bft() {
    ...
    transaction_sender
        .send_vote_cast(&mut alice, &vote_plan, 0, &favorable_choice, &quibitous)
        .unwrap();
    transaction_sender
        .send_vote_cast(&mut bob, &vote_plan, 0, &favorable_choice, &quibitous)
        .unwrap();

    let rewards_before: u64 = quibitous.rest().remaining_rewards().unwrap().into();

    wait_for_epoch(1, quibitous.rest());
    ...
    assert_first_proposal_has_votes(
        ...
        quibitous.rest().vote_plan_statuses().unwrap(),
    );

    let rewards_after: u64 = quibitous.rest().remaining_rewards().unwrap().into();
...
pub fn test_vote_flow_optimum() {
    ...
    transaction_sender
        .send_vote_cast(&mut alice, &vote_plan, 0, &yes_choice, &quibitous)
        .unwrap();
    transaction_sender
        .send_vote_cast(&mut bob, &vote_plan, 0, &yes_choice, &quibitous)
        .unwrap();
    transaction_sender
        .send_vote_cast(&mut clarice, &vote_plan, 0, &no_choice, &quibitous)
        .unwrap();
    ...
    wait_for_epoch(3, quibitous.rest());
...
pub fn qcli_e2e_flow() {
    ...
    let vote_plan_cert = qcli.certificate().new_vote_plan(vote_plan_json.path());

    let tx = qcli
        .transaction_builder(quibitous.genesis_block_hash())
        .new_transaction()
    ...
    let vote_plan_id = qcli.certificate().vote_plan_id(&vote_plan_cert);
    let vote_cast = qcli
        .certificate()
    ...
    let vote_tally_cert = qcli.certificate().new_public_vote_tally(vote_plan_id);
...
pub fn duplicated_vote() {
    ...
    let alice_account_state = quibitous
        .rest()
