...
When starting a genesis optimum blockchain there is an element to take
into consideration while constructing the block 0: _the stake distribution_.

In the context of Genesis/Optimum the network is fully decentralized and it is
...
blockchain_configuration:
  block0_consensus: genesis_optimum
  consensus_genesis_optimum_active_slot_coeff: 0.1
  kes_update_speed: 43200 # 12hours
```

`block0_consensus` set to `genesis_optimum` means you want to start a blockchain with
genesis optimum as the consensus layer.

`consensus_genesis_optimum_active_slot_coeff` determines minimum stake required to
...
follow the instruction in [QCLI's address guide](../qcli/address.md). Utilise the stake key
registered previously as group address:

```sh
qcli address single $(cat wallet_key.pub) $(cat stake_key.pub)
...
```sh
quibitous --genesis-block block-0.bin --config config.yaml --secret poolsecret.yaml
```
