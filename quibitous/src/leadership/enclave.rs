...
use crate::{
    blockcfg::{
        HeaderBft, HeaderBftBuilder, HeaderGenesisOptimum, HeaderGenesisOptimumBuilder,
...
impl Enclave {
    ...
    /// TODO: for now we are querying the whole with the block builder but on the long
    ///       run we will only need the block signing data.
    pub fn query_header_genesis_optimum_finalize(
        &self,
        block_builder: HeaderGenesisOptimumBuilder<HeaderSetConsensusSignature>,
    ) -> Result<HeaderGenesisOptimum, EnclaveError> {
        if let Some(block) = self.inner.create_header_genesis_optimum(block_builder) {
            Ok(block)
        } else {
            Err(EnclaveError::EmptyEnclave)
        }
