...
pub fn test_correct_proposal_number_is_returned() {
    ...
    transaction_sender
        .send_vote_cast(&mut alice, &vote_plan, 0, &Choice::new(0), &quibitous)
        .unwrap();
    transaction_sender
        .send_vote_cast(&mut alice, &vote_plan, 2, &Choice::new(0), &quibitous)
        .unwrap();

    let qcli: JCli = Default::default();
    let rest_uri = quibitous.rest_uri();
    let account_votes = qcli.rest().v1().vote().account_votes(
