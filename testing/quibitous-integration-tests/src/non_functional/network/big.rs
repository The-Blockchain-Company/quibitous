...
use quantricity::builder::{Blockchain, Topology, WalletTemplate};
use quantricity::controller::Controller;
use quibitous_automation::quibitous::{download_last_n_releases, get_quibitous_bin};
use quibitous_automation::quibitous::{QuibitousProcess, PersistenceMode};
...
pub fn real_network(
    ...
    let legacy_app = get_quibitous_bin(last_release, controller.working_directory());
    ...
    let fragment_nodes: Vec<&QuibitousProcess> = leaders.iter().collect();
