...
#### `qcli` TLS requirements.

Note that `quibitous` itself does not have any specific requirements for TLS certificates and you
may give whatever you want including self-signed certificates as long as you do not intend to use
`qcli`.

The cryptography standards used by `qcli` as well as by all modern browsers and many http clients
...
do, `qcli` uses Mozilla's CA bundle for verification) or create your own local CA and provide the
root certificate to `qcli` via the `--tls-cert-path` option.
...
To use the generated certificate, use it and the corresponding key in your `quibitous` config:
...
Use the CA certificate with `qcli`.
...
Right now, as far as we know, only one of them is needed. TBCO provides a few others for
redundancy.
