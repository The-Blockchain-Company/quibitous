use crate::quibitous::genesis::stake_pool::{
...
impl TestnetConfig {
    ...
    pub fn make_leader_config(&self, temp_dir: &TempDir) -> QuibitousParams {
    ...
    pub fn make_passive_config(&self, temp_dir: &TempDir) -> QuibitousParams {
        ConfigurationBuilder::new()
            .with_block_hash(self.block0_hash())
            .with_storage(&temp_dir.child("storage"))
            .with_log(Log(LogEntry {
...
fn create_actor_account(private_key: &str, quibitous: &QuibitousProcess) -> Wallet {
    ...
    let account_state = qcli
        .rest()
        .v0()
        .account_stats(actor_account.address().to_string(), quibitous.rest_uri());
...
fn bootstrap_current(testnet_config: TestnetConfig, network_alias: &str) {
    ...
    let mut quibitous_config = testnet_config.make_passive_config(&temp_dir);

    // start from itn trusted peers
    let quibitous_from_trusted_peers = Starter::new()
        .config(quibitous_config.clone())
        ...
        .start()
        .unwrap();
    quibitous_from_trusted_peers.shutdown();

    quibitous_config.refresh_instance_params();

    // start from storage
    let loading_from_storage_timeout = Duration::from_secs(12_000);
    let quibitous_from_storage = Starter::new()
        .config(quibitous_config)
    ...
    let config = ConfigurationBuilder::new()
        ...
        .with_trusted_peers(vec![quibitous_from_storage.to_trusted_peer()])
        .build(&temp_dir);

    let _quibitous_from_local_trusted_peer = Starter::new()
        .config(config)
...
fn bootstrap_legacy(testnet_config: TestnetConfig, network_prefix: &str) {
    ...
    let mut legacy_quibitous_config = testnet_config.make_passive_config(&temp_dir);

    // bootstrap node as legacy node
    let legacy_quibitous = Starter::new()
        .config(legacy_quibitous_config.clone())
    ...
    let config = ConfigurationBuilder::new()
        .with_block_hash(testnet_config.block0_hash())
        .with_trusted_peers(vec![legacy_quibitous.to_trusted_peer()])
        .build(&temp_dir);

    // bootstrap latest node from legacy node peer
    let new_quibitous_from_local_trusted_peer = Starter::new()
        .config(config)
    ...
    new_quibitous_from_local_trusted_peer.shutdown();
    legacy_quibitous.shutdown();

    // test node upgrade from old data
    legacy_quibitous_config.refresh_instance_params();

    let latest_quibitous = Starter::new()
        .config(legacy_quibitous_config.clone())
        .timeout(Duration::from_secs(48_000))
        .benchmark(&format!(
    ...
    latest_quibitous.shutdown();

    // test rollback
    legacy_quibitous_config.refresh_instance_params();

    let _rollback_quibitous = Starter::new()
        .config(legacy_quibitous_config)
        .timeout(Duration::from_secs(48_000))
        .legacy(version)
        .benchmark(&format!(
...
fn get_legacy_app(temp_dir: &TempDir) -> (PathBuf, Version) {
    ...
    let quibitous = get_quibitous_bin(last_release, temp_dir);
    (quibitous, last_release.version())
}
...
fn e2e_stake_pool(testnet_config: TestnetConfig) {
    ...
    let mut actor_account =
        create_actor_account(&testnet_config.actor_account_private_key, &quibitous);
