...
pub use quibitous_automation::quibitous::{
    grpc::{client::MockClientError, QuibitousClient},
    uri_from_socket_addr, FragmentNode, FragmentNodeError, QuibitousLogger, QuibitousRest,
    MemPoolCheck, RestError,
};
use quibitous_automation::quibitous::{
    QuibitousProcess, ShutdownError, StartupError, StartupVerificationMode, Status,
};
use quibitous_automation::testing::SyncNode;
use quibitous_lib::interfaces::NodeState;
use quibitous_lib::interfaces::{BlockDate, FragmentLog, FragmentsProcessingSummary};
use quibitous_lib::{crypto::hash::Hash, multiaddr};
...
pub struct Node {
    process: QuibitousProcess,
    progress_bar: ProgressBarController,
}

impl Node {
    pub fn new(process: QuibitousProcess, progress_bar: ProgressBarController) -> Self {
        let node = Self {
            process,
            progress_bar,
        };
    ...
    pub fn controller(self) -> QuibitousProcess {
        self.process
    }
    ...
    pub fn logger(&self) -> &QuibitousLogger {
        &self.process.logger
    }
