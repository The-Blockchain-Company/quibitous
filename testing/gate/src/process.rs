...
use quibitous_automation::quibitous::grpc::{
...
use quibitous_automation::testing::{panic, SyncNode};
use quibitous_lib::{
    crypto::hash::Hash,
...
///
/// It uses the same node-to-node gRPC interface as Quibitous (although not complete)
...
/// QuibitousProcess in terms of functionalities, but we start from what is
/// currently needed.
pub struct AdversaryNode {
    ...
    open_client_connections: HashMap<SocketAddr, QuibitousClient>,
}
...
impl AdversaryNode {
    ...
    pub fn send_block_to_peer(
        ...
        let client = self
            ...
            .entry(peer)
            .or_insert_with(|| QuibitousClient::new(peer));
