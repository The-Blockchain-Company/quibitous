...
use quibitous_automation::testing::time::{get_current_date, wait_for_epoch};
...
fn basic_change_config_test() {
    ...
    let old_settings = qcli.rest().v0().settings(quibitous.rest_uri());

    let current_epoch = get_current_date(&mut quibitous.rest()).epoch();

    let wait = Wait::new(Duration::from_secs(5), 10);

    let update_proposal_cert = qcli
        .certificate()
    ...
    let check = qcli.fragment_sender(&quibitous).send(tx.as_str());
    check.assert_in_block_with_wait(&wait);
    let proposal_id = check.fragment_id();

    let update_vote_cert = qcli.certificate().new_update_vote(
    ...
    wait_for_epoch(current_epoch + 2, quibitous.rest());

    let new_settings = qcli.rest().v0().settings(quibitous.rest_uri());
