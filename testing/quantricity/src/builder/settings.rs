...
use quibitous_lib::interfaces::Destination;
use quibitous_lib::interfaces::InitialToken;
use quibitous_lib::interfaces::{try_initial_fragment_from_message, VotePlan as VotePLanLib};
use quibitous_lib::{
    crypto::key::SigningKey,
    interfaces::{
        Bft, Block0Configuration, BlockchainConfiguration, CommitteeIdDef, GenesisOptimum, Initial,
...
impl Settings {
    ...
    fn populate_block0_blockchain_external(
        ...
        for template in external_wallets {
            let address: quibitous_lib::interfaces::Address = template.address().parse().unwrap();
        ...
        RNG: RngCore + CryptoRng,
    {
        ...
        blockchain_configuration.consensus_genesis_optimum_active_slot_coeff =
            *blockchain.consensus_genesis_optimum_active_slot_coeff();
    }
            ...
            if let Some(delegation) = wallet_template.delegate() {
                ...
                let stake_pool_id: StakePoolId = if let Some(node) = self.nodes.get_mut(delegation)
                {
                    if let Some(genesis) = &node.secret.genesis {
                        ...
                        node.secret.genesis = Some(GenesisOptimum {
