...
use quibitous_automation::testing::{
...
pub fn test_100_transaction_is_processed_simple() {
    ...
    qcli.fragments_checker(&quibitous)
...
pub fn test_blocks_are_being_created_for_more_than_15_minutes() {
    ...
    loop {
        ...
        sender.confirm_transaction();
        if let Err(err) =
            super::send_transaction_and_ensure_block_was_produced(&[transaction], &quibitous)
        {
...
pub fn test_expired_transactions_processing_speed() {
    ...
    time::wait_for_date(JLibBlockDate::new(0, 1), quibitous.rest());
    ...
    let summary = fragment_sender
        .send_batch_fragments_in_chunks(transactions, 100, false, &quibitous)
        .unwrap();
