...
use quibitous_automation::{
    quibitous::{Block0ConfigurationBuilder, ConfigurationBuilder, LeadershipMode, Starter},
    testing::keys,
};
use quibitous_lib::interfaces::SlotDuration;
use gate::process::AdversaryNodeBuilder;
...
fn genesis_block_with_incorrect_hash() {
    block_with_incorrect_hash(ConsensusType::GenesisOptimum);
}
...
#[test]
fn genesis_optimum_block0_with_incorrect_hash() {
    block0_with_incorrect_hash(ConsensusType::GenesisOptimum)
}
...
fn block_with_incorrect_hash(consensus: ConsensusType) {
    ...
    let quibitous = Starter::default().config(node_params).start().unwrap();
    ...
    assert!(AdversaryNodeBuilder::new(block0)
        .build()
        .send_block_to_peer(quibitous.address(), block)
        .is_err());
}
