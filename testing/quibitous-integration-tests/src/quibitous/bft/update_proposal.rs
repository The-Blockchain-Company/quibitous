...
use quibitous_automation::testing::keys::create_new_key_pair;
...
fn proposal_expired_after_proposal_expiration_deadline() {
    ...
    let old_settings = quibitous.rest().settings().unwrap();
    ...
    wait_for_epoch(current_epoch + 4, quibitous.rest());

    let new_settings = quibitous.rest().settings().unwrap();
