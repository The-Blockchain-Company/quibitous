use super::{starter::StartupError, QuibitousError};
use crate::qcli::JCli;
use crate::quibitous::grpc::QuibitousClient;
use crate::quibitous::NodeAlias;
use crate::quibitous::StartupVerificationMode;
use crate::quibitous::TestingDirectory;
use crate::quibitous::{
    rest::uri_from_socket_addr, Explorer, QuibitousLogger, QuibitousRest,
    QuibitousStateVerifier, TestConfig,
};
use crate::quibitous::{
    FragmentNode, FragmentNodeError, LogLevel, MemPoolCheck, RemoteQuibitous,
    RemoteQuibitousBuilder,
};
...
use quibitous_lib::interfaces::NodeState;
use quibitous_lib::{
    crypto::hash::Hash,
...
pub struct QuibitousProcess {
    pub child: Child,
    pub logger: QuibitousLogger,
    grpc_client: QuibitousClient,
...
impl QuibitousProcess {
    pub fn new<Conf: TestConfig>(
        ...
        Ok(QuibitousProcess {
            ...
            temp_dir,
            alias,
            grpc_client: QuibitousClient::new(node_config.p2p_listen_address()),
            logger: QuibitousLogger::new(stdout, stderr),
    ...
    pub fn grpc(&self) -> QuibitousClient {
    ...
    pub fn rest(&self) -> QuibitousRest {
        QuibitousRest::new(self.rest_uri())
    }

    pub fn rest_debug(&self) -> QuibitousRest {
        let mut rest = QuibitousRest::new(self.rest_uri());
        rest.enable_logger();
        rest
    }

    pub fn secure_rest<P: AsRef<Path>>(&self, cert: P) -> QuibitousRest {
        QuibitousRest::new_with_cert(self.rest_uri(), cert)
    }

    pub fn shutdown(&self) {
        let qcli: JCli = Default::default();
        qcli.rest().v0().shutdown(self.rest_uri());
    }

    pub fn address(&self) -> SocketAddr {
        quibitous_lib::multiaddr::to_tcp_socket_addr(&self.p2p_public_address).unwrap()
    }

    pub fn correct_state_verifier(&self) -> QuibitousStateVerifier {
        QuibitousStateVerifier::new(self.rest())
    }
    ...
    pub fn check_no_errors_in_log(&self) -> Result<(), QuibitousError> {
        ...
        if !error_lines.is_empty() {
            return Err(QuibitousError::ErrorInLogs {
        ...
        if !stderr.is_empty() {
            return Err(QuibitousError::StdErr { stderr });
        }
        Ok(())
    }
    ...
    pub fn to_remote(&self) -> RemoteQuibitous {
        RemoteQuibitousBuilder::new(self.alias.clone())
...
impl Drop for QuibitousProcess {
    fn drop(&mut self) {
...
impl SyncNode for QuibitousProcess {
...
pub enum ShutdownError {
    ...
    Quibitous(#[from] QuibitousError),
    #[error("process still active")]
    ProcessStillActive(#[from] std::io::Error),
}

impl FragmentNode for QuibitousProcess {
    fn alias(&self) -> NodeAlias {
        self.alias()
    }
